...
from sweep_corner_block_length import sweep_corner_block_length

class TestSweepCornerBlockLength(unittest.TestCase):

    @patch("subprocess.run")
    def test_sweep_corner_block_length(self, mock_run):
        ...
        # Patch open globally so when sweep_corner_block_length calls open("violin_body.json", "r") it gets fake data
        with patch("builtins.open", mock_open(read_data=fake_json_data)):
            results = sweep_corner_block_length()
        ...
        self.assertEqual(first_call_args, ["python3", "cad/violin.py", "--corner_block_length", "10.0"])
