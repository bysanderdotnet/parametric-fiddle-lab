...
def sweep_f_hole_length():
    print("Starting f_hole_length sweep...")
    values = [60.0, 70.0, 80.0, 90.0]
    results = []

    for val in values:
        print(f"Running CAD, meshing, and acoustic simulation with f_hole_length={val}")
        subprocess.run(["python3", "cad/violin.py", f"--f_hole_length", str(val)], check=True, capture_output=True)
        ...
        with open("acoustic_results.json", "r") as f:
            ...
            print(f"f_hole_length: {val} -> A0 Frequency: {freq_str}")

    print("\nSweep Complete. Results:")
    for val, freq in results:
        freq_str = f"{freq:.1f} Hz" if freq else "Not found"
        print(f"f_hole_length={val}: A0={freq_str}")
