...
def sweep_f_hole_bottom_radius():
    print("Starting f_hole_bottom_radius sweep...")
    values = [3.0, 4.0, 5.0, 6.0]
    results = []

    for val in values:
        print(f"Running CAD, meshing, and acoustic simulation with f_hole_bottom_radius={val}")
        subprocess.run(["python3", "cad/violin.py", f"--f_hole_bottom_radius", str(val)], check=True, capture_output=True)
        ...
        with open("acoustic_results.json", "r") as f:
            ...
            print(f"f_hole_bottom_radius: {val} -> A0 Frequency: {freq_str}")

    print("\nSweep Complete. Results:")
    for val, freq in results:
        freq_str = f"{freq:.1f} Hz" if freq else "Not found"
        print(f"f_hole_bottom_radius={val}: A0={freq_str}")

    return results

if __name__ == "__main__":
    sweep_f_hole_bottom_radius()
