import subprocess
from sweep_bridge_thickness import sweep_bridge_thickness

def test_sweep_bridge_thickness(monkeypatch):
    calls = []
    ...
    results = sweep_bridge_thickness()

    assert len(results) == 5
    assert len(calls) == 5
    assert calls[0] == ["python3", "cad/violin.py", "--bridge_thickness", "10.0"]
