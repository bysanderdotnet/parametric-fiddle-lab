...
import sweep_rib_thickness

@patch("subprocess.run")
def test_sweep_rib_thickness(mock_run):
    # Mock subprocess.run to just return a successful result
    mock_run.return_value = subprocess.CompletedProcess(args=[], returncode=0)

    # Mock the JSON file reading
    mock_json_content = '{"eigenmodes": [{"description": "CBR-like", "frequency_hz": 280.0}, {"description": "B1- like", "frequency_hz": 405.0}, {"description": "B1+ like", "frequency_hz": 530.0}]}'

    with patch("builtins.open", mock_open(read_data=mock_json_content)):
        results = sweep_rib_thickness.sweep_rib_thickness()
